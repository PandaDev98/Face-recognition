        ...
        for file in files:            
            try:
                if file.endswith("jpg") or file.endswith("png"):
                    ...
                    img = cv2.resize(img, size)  # resize the image to the desired size
...
try:
    ...
    base_pictures, labels,label_dict = read_images("base_pictures", size=(200, 200))
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            label, confidence = face_recognizer.predict(face)
             ...
             # Determine color for label and rectangle based on whether recognized person is in database
            if label in label_dict.values():
                text_color = known_person_color
                rect_color = known_person_color
                message = "It's Tovs!"
            else:
                text_color = unknown_person_color
                rect_color = unknown_person_color
                message = "Not Tovs!"
            ...
            # Add message below the label
            cv2.putText(frame, message, (x, y+h+30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, text_color, 2)
